...
func NewController(config *convert.WebviewConfig) *Controller {
...
func (sc *Controller) POST(resp http.ResponseWriter, req *http.Request) (name string, data interface{}, render bool) {
	jsonBody, err := inhttp.ExecuteVideosMoverPOST("search", &convert.SearchRequestData{Path: sc.config.DownloadsPath}, sc.config.VideosMoverAPI)
	if err != nil {
		return inhttp.Return500Error("search", err, resp)
	}
	...
	var searchResponseDataList []convert.SearchResponseData
